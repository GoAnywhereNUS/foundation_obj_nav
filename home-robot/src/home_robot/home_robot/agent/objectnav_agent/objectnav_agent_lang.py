...
from home_robot.navigation_planner.language_planner import LanguagePlanner
...
def get_semantic_label_mapping(detector):
    if detector == "detectron2":
        print("Using Detectron2 semantic map")
        return DETECTRON2_LABEL_MAP
    elif detector == "rednet":
        print("Using REDNet semantic map")
        return REDNET_LABEL_MAP
    elif detector == "gt":
        print("Using ground truth semantic map")
        return LABEL_MAP
    else:
...
class ObjectNavAgent(Agent):
    ...
    def prepare_planner_inputs(
        ...
        # import cv2
        # cv2.imwrite("goal_map.png", self.semantic_map.get_goal_map(e) * 255)
        ...
        planner_inputs = [
            ...
            # }
            {
                ...
                "sensor_pose": self.semantic_map.get_planner_pose_inputs(e),
                "found_goal": found_goal[e].item(),
                "object_goal_category": object_goal_category[e].item()
            }
            for e in range(self.num_environments)
        ]
    ...
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        ...
        # Save the goal map, frontier map, and obstacle map
        # import cv2
        # cv2.imwrite(
        #     f"goal_map.png", planner_inputs[0]["goal_map"] * 255,
        # )
        # cv2.imwrite(
        #     f"frontier_map.png", planner_inputs[0]["frontier_map"] * 255,
        # )
        # cv2.imwrite(
        #     f"obstacle_map.png", planner_inputs[0]["obstacle_map"] * 255,
        # )
